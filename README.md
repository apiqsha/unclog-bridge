...
```sh
npx --yes unclog-bridge@1.0.8 connect --tool codex --setup-intent <short-lived-intent>
```
...
Use `npx --yes unclog-bridge@1.0.8 follow` to resume. Hosted Unclog returns the current optimized phase guidance while this package renders all executable commands through the thin bridge. Default command output is compact; manager status/watch keeps the live guidance, actionable signals, notes and commands while repeated assignment/progress projections remain available through `--raw` only. A worker remains pending until its separate thread fetches the focused packet; an unconfirmed stale handoff routes back to one idempotent prompt reissue instead of an endless monitoring loop. Active draft listings add only a bounded preview derived inside the customer repository; unconfirmed goal content is not uploaded for listing or resume. Use `status` to check the connection, `doctor` to inspect local prerequisites, and either `logout` or `revoke` to revoke the current device immediately.
