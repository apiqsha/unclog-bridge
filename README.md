...
```sh
npx --yes unclog-bridge@1.0.9 connect --tool codex --setup-intent <short-lived-intent>
```
...
Use `npx --yes unclog-bridge@1.0.9 follow` to resume. Hosted Unclog returns the current optimized phase guidance while this package renders all executable commands through the thin bridge. Default command output is compact; routine manager status/watch keeps only actionable signals, notes, commands, and a live-guidance identity while the repeated full phase guidance and diagnostic projections remain raw-only. A worker remains pending until its separate thread fetches the focused packet; controller-selected scope never counts as worker activity, and an unconfirmed stale handoff routes back to one idempotent prompt reissue instead of an endless monitoring loop. Hosted watch is always evaluated with the hosted persistence contract. Active draft listings add only a bounded preview derived inside the customer repository; unconfirmed goal content is not uploaded for listing or resume. Use `status` to check the connection, `doctor` to inspect local prerequisites, and either `logout` or `revoke` to revoke the current device immediately.
