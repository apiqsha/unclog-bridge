...
```sh
npx --yes unclog-bridge@1.1.2 connect --tool codex --setup-intent <short-lived-intent>
```

`--tool` may be `codex`, `claude`, `cursor`, or `generic`. The bridge resolves the repository root, waits for approval in the already signed-in dashboard, links the repository, installs the exact public runtime under `~/.unclog/bridge/runtime/1.1.2`, and registers an `unclog` local stdio MCP server without replacing unrelated client settings. Customers do not need an npm account and do not copy or type a device code.
...
If recovery is needed, invoke the exact pinned release shown by the dashboard, for example `npx --yes unclog-bridge@1.1.2 doctor`. Unknown routine CLI commands fail closed and direct the agent back to `unclog_next`.
